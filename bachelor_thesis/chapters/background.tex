...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\textwidth]{abstract_layering_of_coap}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\textwidth]{reliable_message_transmission}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\textwidth]{unreliable_message_transmission}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\textwidth]{piggybacked_response}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\textwidth]{separate_response}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{coap_header}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{coap_message}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{structure_option.png}
...
Im nachfolgenden Listing \ref{listing:synchrone-methode-in-csharp} wird eine Instanz eines \mintinline{csharp}{DownloadClient} erzeugt. Dieser besitzt eine Methode \textit{Download}, mit welcher der Inhalt einer Webseite heruntergeladen und als \mintinline{csharp}{string} zurückgegeben werden kann.
...
Damit der aufzurufende Thread bzw. der Main-Thread nicht andauernd auf die Beendigung der Methode warten muss, kann man Events einsetzen, wie in Listing \ref{listing:eventbasierte-methode-in-csharp} dargestellt. Diese beendigen vorzeitig die Exekution der Methode, indem der Aufrufer ein Objekt übergeben bekommt. Dieses Objekt repräsentiert den derzeitigen Zustand der Operation - also in diesem Fall das Herunterladen des Webseiteninhaltes. Dafür wird in Zeile 3 das \mintinline{csharp}{DownloadResult}-Objekt erzeugt.

In Zeile 5 wird durch den Operator \mintinline{csharp}{+=} die nachfolgende anonyme Lambdafunktion \mintinline{csharp}{(content) => result.SetComplete(content)} als Beobachter des Events \mintinline[breaklines, breakafter=.]{csharp}{client.DownloadComplete} registriert. Diese Lambdafunktion hat die Aufgabe den heruntergeladen Inhalt der Webseite in das \mintinline{csharp}{DownloadResult} zu setzen und als vollständig zu markieren. Dies passiert durch den Aufruf der Methode \mintinline{csharp}{SetComplete(string content)} auf dem \mintinline{csharp}{DownloadResult}-Objekt. Anschließend startet der \mintinline{csharp}{DownloadClient} die Operation mithilfe des Aufrufs \mintinline{csharp}{client.StartDownload(uri)}.
...
Ergänzend zu erwähnen ist, dass eine Methode immer durch die Schlüsselwörter \mintinline{csharp}{async} bzw. \mintinline{csharp}{await} gekennzeichnet wird. Dabei wird nur \mintinline{csharp}{async} vorausgesetzt, wenn innerhalb der asynchronen Methode auf ein Ergebnis einer anderen asynchronen Methode, mittels dem Schlüsselwort \mintinline{csharp}{await}, gewartet wird (siehe Zeile 3 im Listing \ref{listing:asynchrone-methode-in-csharp}).
